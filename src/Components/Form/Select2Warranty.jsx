...
export default function Select2Warranty({ placeHolder, cssClass, value, change, options, click }) {
            ...
            {open && <div className="absolute top-10 left-0 w-full max-h-52 bg-white border z-50">
                <ul>
                    {options && options.map((item) => (<>
                        <li className="bg-white cursor-pointer text-center border-b py-2" value={item.id} onClick={() => { click(item); setInput(item.companyName)  ; setOpen(false)}}>{item.companyName}</li>
                    </>))}
                </ul>
            </div>}
        </div>
