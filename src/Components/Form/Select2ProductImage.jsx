           ...
           {open && <div className="absolute top-10 left-0 w-full max-h-52 bg-white border  z-50">
                <ul>
                    {options && options.map((item) => (<>
                        <li className="bg-white cursor-pointer text-center border-b py-2" value={item.id} onClick={()=>{click(item) ; setInput(item.product.name) ; setOpen(false)}}>{item.product.name}</li>
                    </>))}
                </ul>
            </div>}
        </div>
