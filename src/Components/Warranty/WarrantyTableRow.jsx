...
import { removeWarranty } from "../../Api/Warranty";
import { toast } from "react-toastify";

export function WarrantyTableRow({ editMode, editItem, reload, item }) {
    ...
    async function removeHandle() {
        let response = await removeWarranty(item.id);
        reload();
    ...
    return (
        <>
            ...
            <Tr>
                <Td>{item.id}</Td>
                <Td>{item?.regWarranty?.companyName}</Td>
